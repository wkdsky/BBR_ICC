...
迁移后的目标不是把 Linux 内核代码逐行照搬到仿真器里，而是在 `ns-3` 现有 `DQC/BBRv2` 主机实现上，保留算法语义、控制逻辑和可复现实验路径，使其能够：
