...
################################################################################
# BBRv2 Suite Runner (plan file driven)
#
# Usage:
#   ./run_bbrv2_suite.sh --plan-file <plan.txt> [OPTIONS]
#
# Plan file format:
#   - empty lines / lines starting with # are ignored
#   - default: each line is appended to ./run_bbrv2_batch.sh
#       e.g. --flows 4,8,16,32 --sender-bw 10 --bottle-bw 100 --queue-bdp 4
#   - raw command mode: prefix line with CMD:
#       e.g. CMD: ./run_bbrv2_batch.sh --flows 8 --bottle-bw 20 --sim-time 60
#
# Options:
#   --plan-file <path>        Plan file path (required)
#   --suite-name <name>       Suite folder prefix (default: bbrv2_suite)
#   --instance-base <id>      Auto instance starting value (default: 1)
#   --stop-on-fail            Stop immediately if one case fails
#   --keep-batch-root         Keep per-run traces/bbrv2_batch_ins* roots
...
NS3_DIR="$SCRIPT_DIR"
BATCH_SCRIPT="./run_bbrv2_batch.sh"

PLAN_FILE=""
SUITE_NAME="bbrv2_suite"
INSTANCE_BASE=1
STOP_ON_FAIL=false
...
if [[ ! -f "$NS3_DIR/run_bbrv2_batch.sh" ]]; then
  echo "Error: $NS3_DIR/run_bbrv2_batch.sh not found"
  exit 1
fi
...
while IFS= read -r raw_line || [[ -n "$raw_line" ]]; do
  ...
  if [[ "$cmd_line" == *"run_bbrv2_batch.sh"* && "$cmd_line" != *"--instance"* ]]; then
