 ...
 * n3----L3--------|                    |------L8-------n9
 *
 * 4 FBBR flows, 8Mbps bottleneck, 60ms RTT, buffer = 1xBDP
 */
...
#define DQC_SCENARIO_LOG_COMPONENT "fbbr-4flow"
#endif
#ifndef DQC_SCENARIO_INSTANCE
#define DQC_SCENARIO_INSTANCE "fbbr_4flow"
#endif
#ifndef DQC_SCENARIO_TITLE
#define DQC_SCENARIO_TITLE "4 FBBR Flows"
#endif
...
// FBBR oscillation parameters for each flow
...
void ApplyFBBRConfigToGlobals(const FBBRConfig& config)
{
...
void ApplyGlobalsToFBBRConfig(FBBRConfig* config)
{
    ...
    for(uint32_t i = 0; i < NUM_FLOWS; ++i){
        FBBRFlowConfig& flow = config->flow[i];
...
std::string PreScanFBBRConfigPath(int argc, char* argv[],
                                     const std::string& default_path)
{
    const std::string fbbr_prefix = "--fbbrConfig=";
    for(int i = 1; i < argc; ++i){
        const std::string arg = argv[i];
        if(arg.rfind(fbbr_prefix, 0) == 0){
            return arg.substr(fbbr_prefix.size());
        }
        if(arg == "--fbbrConfig" && i + 1 < argc){
            return argv[i + 1];
        }
    }
    return default_path;
	                        ...
	                        uint32_t flow_index=0)
{
    ...
    if(trace){
        ...
        if(trace_enable & (DqcTraceEnable::E_DQC_FBBR_LOAD |
                           DqcTraceEnable::E_DQC_FBBR_GATE)){
        sendApp->SetFBBRLoadTraceFuc(MakeCallback(&DqcTrace::OnFBBRLoad,trace));
        }
        ...
        sendApp->SetLossRateTraceFuc(MakeCallback(&DqcTrace::OnLossRate,trace));
		    ...
		    if(cc_type == kFBBR || cc_type == kFBBRAdaptive ||
               ...
               cc_type == kFBBRHybridV4 || cc_type == kFBBRServiceFair){
...
void ns3_fbbr(int ins, std::string algo, DqcTraceState *stat, double sim_time=60.0, int loss_integer=0){
    ...
    const std::string algo_key = Trim(algo);
    ...
    if(algo_key == "BBRv2"){
        ...
        NS_ABORT_MSG("unsupported algorithm: " << algo
                                               << " (supported: FBBR, "
                                                  "FBBR-adaptive, FBBR-hybrid, FBBR-hybridv3, FBBR-hybirdv4, FBBR-ServiceFair, BBRv2)");
    }
...
int main (int argc, char *argv[]){
    ...
    std::string algo="FBBR";
	    ...
	    CommandLine cmd;
	    cmd.AddValue("fbbrConfig", "FBBR key=value config file path", g_fbbr_config_path);
    ...
    cmd.AddValue("algo", "Congestion control: FBBR, FBBR-adaptive, FBBR-hybrid, FBBR-hybridv3, FBBR-hybirdv4, FBBR-ServiceFair, or BBRv2", algo);
    ...
    cmd.AddValue("seed", "ns-3 RNG seed", seed);
	    ...
	    cmd.AddValue("enableCruiseWindowTrace", "Enable FBBR CRUISE window trace", g_enable_cruise_window_trace);
    ...
    cmd.AddValue("gateTraceMode", "FBBR gate trace mode: off, round_only, sampled_pacing, full", g_gate_trace_mode);
	    ...
	    cmd.AddValue("waveformClipMinHalfOverlapRatio", "Minimum clipping overlap with one sender half-cycle", g_fbbr_config.waveform_clip_min_half_overlap_ratio);
    ...
    for (size_t c = 0; c < sizeof(algos) / sizeof(algos[0]); ++c){
        ...
        for (size_t i = 0; i < sizeof(ins) / sizeof(ins[0]); ++i){
            ns3_fbbr(ins[i],cong,stat.get(),sim_time);
        }
