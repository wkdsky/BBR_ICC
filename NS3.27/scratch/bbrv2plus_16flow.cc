...
#define DQC_SCENARIO_LOG_COMPONENT "bbrv2plus-16flow"
#endif
#ifndef DQC_SCENARIO_INSTANCE
#define DQC_SCENARIO_INSTANCE "bbrv2plus_16flow"
#endif
#ifndef DQC_DEFAULT_CC
#define DQC_DEFAULT_CC "bbrv2plus"
#endif
#ifndef DQC_SCENARIO_TITLE
#define DQC_SCENARIO_TITLE "16 BBRv2plus Flows"
#endif
...
void ns3_bbrv2plus(int ins, std::string algo, DqcTraceState *stat, int sim_time=60, int loss_integer=0){
    std::string instance=algo + "_16flow";
    ...
    queue_tracers.push_back(InstallBottleneckQueueOccupancyTrace(devn16n17.Get(0), instance, NUM_FLOWS));
    ...
    dqc::CongestionControlType cc = kBBRv2Plus;
    if (algo == "bbrv2plus_ecn") {
        cc = kBBRv2PlusEcn;
    }
...
int main (int argc, char *argv[]){
    ...
    cmd.AddValue("cc", "Congestion control: bbrv2plus, bbrv2plus_ecn", cc_name);
    cmd.Parse(argc, argv);
    if(!trace_path.empty()){
        if(trace_path.back() != '/'){
    ...
    for (size_t c = 0; c < algos.size(); ++c){
        ...
        for (size_t i = 0; i < sizeof(ins) / sizeof(ins[0]); ++i){
            ns3_bbrv2plus(ins[i],cong,stat.get(),sim_time);
        }
