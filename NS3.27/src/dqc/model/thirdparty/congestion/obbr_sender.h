...
namespace dqc{
class RttStats;
class ObbrSender : public SendAlgorithmInterface {
 public:
  enum Mode {
 ...
 // Debug state can be exported in order to troubleshoot potential congestion
  // control issues.
  struct DebugState {
    explicit DebugState(const ObbrSender& sender);
  ...
  ObbrSender(ProtoTime now,
            const RttStats* rtt_stats,
            const UnackedPacketMap* unacked_packets,
            QuicPacketCount initial_tcp_congestion_window,
            QuicPacketCount max_tcp_congestion_window,
            Random* random,bool drain_to_target);
  ObbrSender(const ObbrSender&) = delete;
  ObbrSender& operator=(const ObbrSender&) = delete;
  ~ObbrSender() override;
 ...
 private:
  ...
  QuicByteCount GetQuantum() const;

  // Called right before exiting STARTUP.
  void OnExitStartup(ProtoTime now);
  void ResetObbrScoreWindow(ProtoTime now);
  void UpdateObbrState(ProtoTime now,
  ...
  int obbr_cc_stage_;
  bool obbr_reset_score_window_;
  ProtoTime obbr_score_time_;
  ProtoTime obbr_loss_timestamp_;
  ProtoTime obbr_last_revert_time_;
  QuicBandwidth obbr_saved_bandwidth_;
  QuicByteCount obbr_score_sent_base_;
  QuicByteCount obbr_score_delivered_base_;
  int64_t obbr_score1_;
  int64_t obbr_score2_;
  int64_t obbr_score3_;
  int64_t obbr_score4_;
};

 std::ostream& operator<<(std::ostream& os,
                                             const ObbrSender::Mode& mode);
 std::ostream& operator<<(
    std::ostream& os,
    const ObbrSender::DebugState& state);
}
